...
const Home = () => {
  ...
  return (
    <div ref={containerRef} className={`min-h-screen transition-colors duration-300 ${darkMode ? 'dark bg-space-900' : 'bg-white'}`}>
      {/* Dark Mode Toggle */}
      <motion.button
        className="fixed top-4 right-4 z-50 p-2 rounded-full bg-space-100 dark:bg-space-800 shadow-lg"
        ...
        <AnimatePresence mode="wait">
          {darkMode ? (
            ...
            <motion.div
              ...
              transition={{ duration: 0.3 }}
            >
              <Moon className="w-6 h-6 text-space-600" />
            </motion.div>
          )}
        </AnimatePresence>
      </motion.button>
